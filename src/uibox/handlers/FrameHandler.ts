...
class FrameHandler extends BaseHandler {
  ...
  getOptions = (): FrameOptions => {
    const frame = this.get()
    // Get the JSON representation of the frame
    const frameJSON = frame.toJSON(this.root.propertiesToInclude)
    
    // Create a properly typed FrameOptions object that meets the interface requirements
    const options: FrameOptions = {
      id: '', // Default ID
      name: 'Frame', // Default name
      width: 1280, 
      height: 720,
      fill: '#ffffff',
      ...frameJSON // Spread any other properties
    }
